...
pub trait ShieldedTokenInterface {
    ...
    /// max(0, shielded_supply - custodied balance): how much less the pool holds than it owes.
    fn custody_shortfall(env: Env, asset: Address) -> i128;
...
**Clawback policy.** A non-native SEP-41 asset's issuer can claw back any holder's balance, including this contract's custodied balance after a deposit. The contract handles it in three layers: (1) *Prevention*: an asset can only be shielded after an explicit governance approval (`set_asset_approved`), so accepting an issuer's clawback right is an on-chain, auditable decision; native XLM has no issuer and is safe to approve. (2) *Detection*: `custody_shortfall(asset)` reports `max(0, shielded_supply - custodied balance)` and is zero while the pool is fully backed; a positive value means funds were clawed back. (3) *Loss sharing*: when the pool holds less than it owes, `unshield` pays `pub_value * balance / supply` (both measured before the withdrawal) instead of `pub_value`, so the loss is shared pro rata by every holder rather than going to whoever withdraws first, and the backing ratio stays constant for those still in the pool. Governance's response is to revoke the asset (`set_asset_approved(asset, false)`), which stops new deposits without trapping existing notes. `contracts/swap::commit_swap` refuses to commit when the unshield it triggers pays less than `amount_in` (a partial escrow would strand the counterparty), leaving the note unspent. Tested by `unshield_shares_a_clawback_loss_pro_rata_and_reports_the_shortfall` against a Stellar Asset Contract with clawback enabled.

**Measured instruction costs (real compiled WASM, 400M mainnet limit).** shield 118.6M (29%), transfer 2x2 233.8M (58%), transfer4 397.9M (99.5%, thin margin), unshield 34.1M (8.5%), shield_batch of 3 347.2M (87%, hence `MAX_SHIELD_BATCH = 3`); verifier `verify` 29.2M, `verify_batch` of 4 63.0M. Each is checked against its native-Rust figure on every CI run (`contracts/token/src/tests/cost_parity.rs`, `contracts/verifier` `cost_parity_verify_and_verify_batch`); the build fails if a WASM figure exceeds 400M or is more than 25% above native. The batched multi-scalar-multiplication in `verify` was measured end to end on transfer4 against the per-input loop it replaced: 412.1M with the loop (over the limit) versus 397.9M with the MSM. Batch verification against verifying one by one: 2 proofs 43.4M vs 58.4M, 4 proofs 63.0M vs 116.7M, 8 proofs 102.3M vs 233.4M.
