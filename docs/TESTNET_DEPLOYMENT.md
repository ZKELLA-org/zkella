...
## Update: Tranche 1 live validation stack (transfer4 and unshield on the current build)

A fresh, isolated verifier + token pair built from the current source (batched-MSM verifier, asset allowlist, VK retention window). The verifier's admin is the deployer directly, not `governance`, so this stack validates the proving and on-chain verification path, not the governance timelock (exercised on the stack above). It is a validation deployment, not a replacement for the live contracts listed at the top of this document.

| Contract | Address |
| --- | --- |
| verifier | `CAQXCXYB7EW7WW7AGE7UKQNRS7SDJYTDEXOIMSBBUTE6JWTLH5MVMMLJ` |
| token | `CB3MI4XWPUILARKYR2XQVINU6AOIMHJYH3V5BZ2J7UBNY3CLWJBPMLPR` |

Produced by `scripts/testnet_live_validation.cjs`, each proof generated by the SDK and submitted as its own transaction:

| Step | Result | Tx |
| --- | --- | --- |
| `shield` (leaf 1) | real Groth16 verified on-chain | https://stellar.expert/explorer/testnet/tx/66b1ca7a6e07de283f96ce5410006594bfb2f434df3c4ed8210432f8f154a7ca |
| `shield` (leaf 2) | | https://stellar.expert/explorer/testnet/tx/3b4440fe15ca7d951014f0c1a51d2ec704265e8d4b835796c2586525762feb4e |
| `shield` (leaf 3) | | https://stellar.expert/explorer/testnet/tx/acda1a1428e96731ea5a0ab82b2d97a04b42e2493b46c4a95178b986fd4dc508 |
| `shield` (leaf 4) | | https://stellar.expert/explorer/testnet/tx/f63dcef8cd1911803931965d1ea98e6925931ec1d3448e1a15431eaa859a6a8b |
| `transfer4` (4-in/4-out, new leaves 5-8) | real 19-signal Groth16 verified on-chain within the network's instruction limit | https://stellar.expert/explorer/testnet/tx/292ae9dba32fcee412b6c79e910507b03613bfca32e26db9de314c7647e21609 |
| `unshield` | real Groth16 verified on-chain; shielded supply fell from 5,000,000 to 4,000,000 | https://stellar.expert/explorer/testnet/tx/87b51db06cee71c99b7f6c08b503e38285c00f4a1680fbb98b38128a4f290a91 |

Leaf 0 is an earlier shield whose result the client could not parse (the SDK's XDR decoder predated the current protocol); it landed on-chain and its note was not tracked. That is why supply started at 5,000,000. Fixing it required upgrading `@stellar/stellar-sdk` from 12 to 17.

The swap-fairness circuit was validated live on the earlier stack above (`reveal_and_claim`); it has not been re-run against this validation stack.
