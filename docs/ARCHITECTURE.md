...
- `shield(from, asset, amount, rho, rcm, owner_pk, commitment, encrypted_note, shield_proof, shield_pub)`
- `shield_batch(from, asset, items)` — at most `MAX_SHIELD_BATCH = 3` items (`BatchTooLarge = 21`, `EmptyBatch = 20`); each `ShieldBatchItem` carries `owner_pk` after `rcm`
- `set_asset_approved(asset, approved)` / `is_asset_approved(asset)` — shielding an unapproved asset fails with `AssetNotApproved = 18`; `set_min_shield_amount` / `min_shield_amount`
...
**ShieldedToken — transfer.** Built and proven live for the 2-in/2-out path: a genuine proof, generated through the SDK's own `generateTransferProof` using Merkle paths read from the deployed contract, was submitted and verified on-chain at https://stellar.expert/explorer/testnet/tx/90fe4d1996815f77c7c87b06a29141e01fab293dc44d01b56364be7c7e4fcf14, correctly spending two input notes and inserting two new output notes. The 4-in/4-out variant's verifying key is also live-registered (queued at https://stellar.expert/explorer/testnet/tx/3c8fb0c8bcff1a8e6fbe99be14f4b6af9cb0473280858a6be3d8999621ac5fa8, executed at https://stellar.expert/explorer/testnet/tx/320c4f2058df75acbd837b78dcaa5ede7bca1422aee708c53a9deea6bbc2ea59). Remaining: `fee` is fully constrained in-circuit (`Σ in_value = Σ out_value + fee`) but nothing in the contract actually pays it to anyone today — see §4.2's note on the planned relayer-fee design; the 4-in/4-out path has been run live once, on the Tranche 1 stack (tx `a7858b390a6351d7ef8798fce58af377c16f956f98896071fb972cb02c3503cf`). Its real compiled-WASM cost was originally measured at roughly 97% of the mainnet instruction budget; re-measuring against current stable Rust toolchains (1.92.0 through 1.94.1) showed it landing marginally *over* the 400M budget on every one of them (see `docs/SCF_READINESS.md` for the full comparison). The verifier's batched multi-scalar-multiplication optimization (below) has since landed and been re-measured: this same call now uses 396,688,826 instructions, 99.17% of the mainnet budget, back under the line but with only ~0.83% headroom, thin, not comfortable. The optimization was a real prerequisite and it worked, but this path is not on a safety margin comparable to shield's or 2-in/2-out transfer's yet.

**ShieldedToken — unshield.** Built and proven live as a sub-step of the swap's commit flow: https://stellar.expert/explorer/testnet/tx/21c4380b39685c9674edabb2f2830d931e8ead0d557adcff1a4aecdf66bc8038. Remaining: `unshield` takes exactly one nullifier and produces no output commitment, so a user cannot unshield part of a note's value while keeping the rest shielded as change, today requiring transfer-then-unshield as two separate operations; a standalone, directly-invoked unshield has been run once on the Tranche 1 stack (tx `668fa5bfe469b28983c710f7a448b825c633e0f97e94992f0f3c4c21665fc334`; 33,887,174 instructions, 8.5% of the limit).

**Verifier registry.** Built and proven live: the on-chain Groth16/BN254 pairing check has cleared real shield proofs (https://stellar.expert/explorer/testnet/tx/0722df0e01bd81ee256fb317c44a97a4e713c19fe019e27460216887fb7cacee) and a real transfer proof (https://stellar.expert/explorer/testnet/tx/90fe4d1996815f77c7c87b06a29141e01fab293dc44d01b56364be7c7e4fcf14). Public-input aggregation used to loop individual point-multiplication and point-addition host calls once per input; it now computes the same `vk_x = IC[0] + Σxᵢ·IC[i+1]` sum via a single `bn254_g1_msm` batched call (available since Protocol 26), verified correct against all five real, live-circuit proofs the verifier's own test suite checks (shield, transfer_2in2out, transfer_4in4out, unshield, swap_fairness, 17 tests total, all passing unchanged), and measured to save roughly 3.3 million instructions on transfer4's 19-input case, bringing that path from over budget back to 99.17% of the mainnet limit. Remaining: that margin is thin (~0.83% headroom), not a comfortable one, and further optimization is a reasonable next step, not a closed item; `update_verifying_key` overwrites the old key the instant a rotation executes with no retention window, so a proof built against the pre-rotation key simply fails if submitted after; and there is no batch-verification path for checking several proofs with one combined pairing check.
...
**SDK.** Built and proven live: the base wallet's shield and transfer paths work and have each produced a proof used in a real, on-chain-verified transaction (the two URLs above for shield and transfer). Remaining: the wallet's submission path throws on a timeout with no retry logic for a transient RPC failure or an anchor aging out of the root-history window; a browser Web Worker prover exists (`sdk/src/prover/worker.ts`; Node `worker_threads` cannot run snarkjs), checked once in headless Chromium (main-thread worst stall about 205 ms inline versus about 19 ms with the worker; manual check, not in CI); and the `ZKELLASwap`/`ZKELLAAuditor`/`ZKELLACompliance` wrapper classes are still stubs returning placeholder values, with nothing published to npm under any tag.
...
- **One dedicated Confidential Vault wrapper contract per target vault**, not a single generic router that can point at many targets interchangeably. This matches Zama's own precedent directly: their cUSDC vault is a specific wrapper built for Steakhouse's Prime v2 strategy on Morpho, not a general-purpose adapter for any Morpho vault. Each wrapper is bound to exactly one counterpart contract address at deployment/initialization, the same pattern `contracts/swap` itself already uses for its own `token_contract` reference, a fixed reference set once, not a caller-suppliable parameter, and not a dynamic allowlist. Integrating a second target vault means reviewing and deploying a second, independent wrapper contract, not reconfiguring the first one. This is deliberately more conservative than reusing `contracts/swap`'s relayer-authorization pattern would be: a relayer allowlist answers *who* may act on the contract's behalf, a fundamentally different question from *which external contract* a wrapper is allowed to move real custodied funds into, and collapsing the two into one mechanism would be a mismatch, not a simplification.
- **Vault-share notes**: a shielded deposit into the vault produces a note whose value represents a *share count* in that one specific vault, not a raw asset amount. Each vault wrapper maintains its own instance of the same commitment/nullifier/Merkle-tree pattern `contracts/token` already implements for ordinary shielded notes, a separate tree scoped to that vault's shares, not `contracts/token`'s own tree. No new note primitive is needed, only a new circuit that proves the deposit and share-count arithmetic correctly.
...
- **Each target vault is a separate review and deployment decision, not a configuration change.** Because a wrapper is bound to one counterpart contract for its lifetime, adding support for a new underlying vault means independently auditing that specific external protocol and shipping a new wrapper contract for it, real recurring engineering and review work, not a one-time mechanism that then generalizes for free.
