...
`contracts/ct20/shield()` now also performs **real on-chain Groth16/BN254 proof verification** via a cross-contract call to a new `contracts/verifier` registry contract (see "Implemented components" below), rather than the `TODO(M2)` stub this section originally described.
...
- Regression test: `contracts/ct20/src/lib.rs`'s `shield_fits_within_mainnet_instruction_budget`.
...
All six contracts (`verifier`, `governance`, `compliance`, `viewing_keys`, `ct20`, `swap`) were deployed and wired together on live Testnet:
...
| ct20 | `CBAI76M764AFB5JQ3VFAUTIX6MBICDYMVWALV5IXCM6KSEGU5LHL2BZ7` |
| swap | `CDVGPM7LBZZAOEYFPZGANCYMRQPAKMCUJZTMODCPHVLU53IHNDAZVOQ6` |

`verifier`'s admin is `governance`'s own contract address (self-authorizing pattern: cross-contract calls from `governance` satisfy `verifier`'s `admin.require_auth()` without a signature); `ct20`, `governance`, and `compliance` all point at the same `verifier` instance. The real `shield.circom` verifying key was registered on-chain via `governance.register_vk`.
...
(`https://stellar.expert/explorer/testnet/tx/<hash>` for each.) Post-run state: `leaf_count() = 3`, `shielded_supply(native) = 60000000` (6 XLM total, matching the three deposits), `merkle_root()` non-empty and changed after each call. Each transaction's on-chain event log confirms the real token `transfer` event from the deployer to the `ct20` contract for the exact shielded amount.
...
| --- | --- | --- |
| 1. Shield | Real `shield.circom` proof; 5,000,000 stroops native XLM shielded into `ct20` (leaf 1) | `09337d4f15c659dcc45d004caa7c423b0984b501168b97482b2abc0bd4f91944` |
| 2. `commit_swap` | Real `unshield.circom` ownership proof for that note; cross-call into `ct20::unshield` verified it on-chain and atomically escrowed the 5,000,000 stroops into `swap`'s own balance | `0bfb955de61f19564d793ce62dd21d12c5fc5b95b0a58bd81aa63128bbe2a971` |
...
- `contracts/ct20/src/lib.rs`
  - `initialize()` stores the address of a `contracts/verifier` registry contract (the verifying key itself lives there, not in `ct20`, so it can be rotated without redeploying the token contract) and instance storage TTL handling
...
- the Rust contracts workspace (`cargo test --workspace`) has 62 passing tests across `ct20` (32), `verifier` (17), `compliance` (3), `governance` (3), `swap` (5), and `viewing_keys` (2), including:
...
- **`reveal_and_claim`** verifies the fairness proof (as before), then releases real value on both sides: pays the relayer the escrowed `asset_in`, and re-shields the escrowed `asset_out` as a fresh note for the claimant via a **real, separate** `ct20::shield` call (`shield_proof` is a genuine `shield.circom` proof for the *output* note — distinct from the fairness proof, which proves a different thing). Returns the real leaf index `ct20` assigns, not a placeholder.
...
Cross-calling `ct20` from `swap` needed a new `contracts/ct20-interface` crate (mirroring `contracts/verifier-interface`'s established pattern exactly, for the identical reason: depending on `zkella-ct20` directly risks a WASM `duplicate symbol` linker error, since Soroban contract exports are unconditional). Getting the test suite to deploy a *real* `ct20` instance to exercise this also surfaced a real, unrelated packaging bug: `ct20`'s crate only built as `cdylib`, never `rlib` — meaning it (like `verifier` before the `verifier-interface` split) could never actually be used as a Rust dependency by anything, including its own dev-dependents. Fixed by adding `rlib` to its `crate-type`, matching `verifier`'s existing pattern. `contracts/swap/src/tests`'s `full_swap_lifecycle_moves_real_value` test exercises the entire flow — commit, execute, claim — with real (synthetic-relation, not real-circuit) proofs and asserts actual SEP-41 token balances move as expected at every step; `cancel_swap_refunds_escrowed_asset_in` and `reclaim_expired_swap_refunds_both_sides` cover both unwind paths.
...
`indexer/` is a real, running reference implementation of the indexer `docs/ARCHITECTURE.md` describes as necessary (Stellar RPC's own event retention window is too short for wallets to recover note/nullifier history beyond it). It polls `SorobanRpc.Server.getEvents` for `ct20`'s `("zkella","note")`/`("zkella","nf")` events, persists them in SQLite (Node's built-in `node:sqlite`, no external database dependency), and serves them over HTTP in exactly the shape `sdk/src/indexer/client.ts`'s `IndexerClient` already expected — `merkle_root`/`merkle_path` are proxied live to `ct20` itself rather than duplicated, since the contract is already the source of truth for current tree state.
...
Not yet covered: horizontal scaling, multiple independent operators, and an operational runbook (alerting on sync failures, backfill tooling) — this is a correct, working reference implementation of the API surface, not a production deployment. `node:sqlite` is still an experimental Node API as of the version used here.
...
- indexer horizontal scaling, multi-operator support, and an operational runbook (alerting, backfill tooling) — the indexer itself is real and live-Testnet-validated (see above), this is about running it at production scale
- a dedicated operational runbook and incident-response plan (deployment, monitoring, rollback, key handling, and escalation paths for contract failures, indexer outages, key exposure, and misconfiguration) — not yet written as a standalone document
...
- a dedicated unit-test regression for the nested cross-contract authorization fix in `reveal_and_claim` (see "Update: senior audit..." above) — currently validated only by the real live-Testnet transactions (pre-fix failure, post-fix success), not by an automated test, since the existing suite's blanket `mock_all_auths_allowing_non_root_auth()` cannot exercise `authorize_as_current_contract` correctness by design
...
- a soft PoC implementation with a working core: shield/transfer/unshield with real on-chain Groth16 verification and real Soroban RPC submission, a shielded swap primitive that genuinely moves value (reusing `ct20`'s own shield/unshield paths), a real BN254 ECDH/hash-to-curve key and encryption layer including diversified addresses, and a real (if reference-scale) persistent indexer — all validated both locally and with real transactions on live Stellar Testnet
