...
**Validated locally only — not yet redeployed or re-run on live Testnet.** The `token` contract currently live on Testnet (`deployments.json`, `CDE7U6HTLMDFAEQOT5BIZ3W7VJKAQN2MFQKYVV5E3W5YIPUBSRBHAXCE`, deployed 2026-08-03) **predates this fix** — it still enforces exact-root equality, not the 32-root window described above. This fix exists in source only as of this writing, verified via `cargo test --workspace` (Soroban's real host environment, run locally) and the WASM release build, but has not been exercised against real concurrent activity on live Testnet the way the original shield-budget and swap-lifecycle work was. Redeploying `token` (and updating `deployments.json`/`docs/TESTNET_DEPLOYMENT.md` accordingly) remains open before this can be described as live-validated rather than locally-validated.
...
**Not yet redeployed.** Every fix above exists in source only as of this writing — the same caveat as the root-history window fix immediately above: the contracts currently live on Testnet predate all of it. See `docs/TESTNET_DEPLOYMENT.md`'s drift note for the full, updated list.
